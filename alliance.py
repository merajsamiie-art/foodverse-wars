...
def create(user_id: int, chat_id: int, name: str) -> tuple:
    ...
    if player.get(user_id)["fc"] < ALLY_CREATE_COST:
        return False, f"🪙 تأسیس اتحاد {ALLY_CREATE_COST:,} فودکوین می‌خواهد."
    with db.db().tx():
...
def betray(user_id: int) -> tuple:
    ...
    msg = (f"🗡 <b>خیانت!</b>\n"
           f"{p['avatar']} {p['name']} اتحاد «{a['name']}» را لو داد و 🪙 {fc_steal:,.0f} فودکوین برداشت!\n"
...
def status_text(user_id: int) -> str:
    ...
    if not a:
        return (f"🤝 <b>بی‌اتحاد</b>\n"
                f"تأسیس ({ALLY_CREATE_COST:,} فودکوین): «تأسیس [نام]» | عضویت: «عضویت [نام]»")
    ...
    return (f"🤝 <b>اتحاد {a['name']}</b>\n"
            f"🏦 خزانه: 🪙 {a['treasury_fc']:,.0f} فودکوین | {res_s}\n"
