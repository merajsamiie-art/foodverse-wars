...
CREATE TABLE IF NOT EXISTS accounts(
  ...
  banned INTEGER DEFAULT 0, created_at REAL, last_active REAL, boss_streak INTEGER DEFAULT 0
);
-- عضویت در دنیاها
...
class DB:
    def __init__(self, path: str):
        ...
        # 🐕 مینی‌باس + 🔥 زنجیره‌ی شکار: ستون‌های جدید
        for _alt in ("ALTER TABLE worlds ADD COLUMN mini_next REAL DEFAULT 0",
                     "ALTER TABLE accounts ADD COLUMN boss_streak INTEGER DEFAULT 0"):
            try:
                self.conn.execute(_alt)
            except Exception:
                pass
