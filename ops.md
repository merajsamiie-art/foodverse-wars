...
| `setChatTitle/Photo/Description` | ✅ با ادمینی بات | ✅ کانال برند شد + پست پین‌شده؛ گروه با «fw مدیر برند» |
...
5. **گروه — برندینگ بدون سورس:** ادمین هر پیام متنی را از گروه نبرد به بات **فوروارد** می‌کند → chat_id در kv (`brand_target`) ثبت می‌شود → بعد «**fw مدیر برند**» عنوان/عکس/توضیحات/پست معرفی+پین را می‌گذارد
...
- 💰 **درآمدهای خرد فعالیتی**: «fw شیفت» (هر ۳ ساعت، ۱۲۰–۲۸۰ سکه+منبع) | «fw گشت» (هر ۴۵ دقیقه، یافته‌های کوچک) | مأموریت روزانه ۶تایی شد (+ خرید/فروش صرافی)
...
- 🧟 **اینفکتد**: آسیب‌برترِ هر رید تا ۱۰ دقیقه می‌تواند باس را با «fw اینفکت» اسیر کند (سطح ۱۰+ و ۱۵۰۰ سکه) — باس از استخرِ رید آن دنیا خارج می‌شود؛ هر ۳ روز باید تازه شود وگرنه آزاد؛ «fw هجوم [ریپلای]» = غارت کوچک + کنترل ساعتی قربانی (۴/۸/۱۲ ساعت بر تیر) — ۱۵٪ تولید قربانی به کنترل‌کننده می‌رسد؛ شکست کنترل‌کننده در هر جنگی کنترل را می‌شکند
