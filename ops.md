...
## ⚠️ کارهای دستی که فقط از تلگرام ممکن است (Bot API راهی ندارد)
1. **Privacy mode را خاموش کن (مهم‌ترین):** @BotFather → `/setprivacy` → انتخاب @FoodverseWarsBot → **Disable** → بعد بات را از گروه **یک‌بار remove و دوباره add کن** (تنها آن‌وقت اعمال می‌شود). تا آن‌وقت: همه‌ی دستورها با `/` هم کار می‌کنند (مثل «/جنگ علی») ولی «fw جنگ» فقط در پیوی دیده می‌شود.
2. **بات را ادمین گروه کن** (راه جایگزین ۱ — ادمین‌ها همه‌ی پیام‌ها را می‌بینند).
3. **بات را ادمین کانال @FoodverseWars کن** تا پست بگذارد.
4. **آواتار بات:** @BotFather → `/setuserpic` (تصویر `brand/bot_avatar.png` بعداً تولید می‌شود).
5. **نام صاحب کارت** برای رسیدها: `PAYMENT_HOLDER` — به من بگو تا سیکرت/تنظیم شود (الان کارت بدون نام نمایش داده می‌شود).
