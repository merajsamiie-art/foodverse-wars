...
- 💰 **درآمدهای خرد فعالیتی**: «شیفت» (هر ۳ ساعت، ۱۲۰–۲۸۰ سکه+منبع) | «گشت» (هر ۴۵ دقیقه، یافته‌های کوچک) | مأموریت روزانه ۶تایی شد (+ خرید/فروش صرافی)
- 👹 **تیر باس**: هر دنیا بر اساس فعالیتش باس معمولی 🥉 / شدید 🥈 / کابوس 🥇 می‌گیرد — جان +۷۵٪ و جایزه +۶۰٪ به‌ازای تیر؛ اسپاون اجباری ادمین = همیشه کابوس
...
- 🗄 مهاجرت‌های امن: ستون‌های جدید با ALTER (بدون reset) — worlds(boss_tier,boss_pool) / accounts(controlled_by,controlled_until) / daily(sold,bought) + جدول infected
...
- **گروه:** `شروع` (۴ نفر) | `پایگاه` | `ارتقا [ساختمان]` | `مستعمره [بازیکن]` | `غارت [بازیکن]` | `جذب [واحد] [تعداد]` | `جنگ [بازیکن]` | `باس` (حمله) | `ساخت [آیتم]` | `تفریخ` | `تجهیز [آیتم]` | `بازار` | `خرید [منبع] [تعداد]` | `فروش [منبع] [تعداد]` | `بفروش [آیتم] [قیمت]` | `برداشتن [شماره]` | `قیمت‌ها` | `تأسیس/عضویت/ترک/خیانت/کمک` | `روزانه` | `فروشگاه` | `تخفیف‌ها`
